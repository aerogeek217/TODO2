...
│   ├── DashboardView      → Renders pinned `listDefinitions` (defaults seeded Today/Upcoming/Deadlines/Someday) via `services/dashboard-lists`; per-list grouping + Taskboard; "Add list" tile → `ListDefinitionPickerPopup`; shares `showCompleted` / `showHiddenStatuses` toggles, ignores other filter-bar fields
│   ├── ListView           → Unified list with sort-by grouping (Date/Scheduled/Deadline/People/Tag/Project/Status/Org), saved views, "Save as Preset" → ListDefinition, "Editing preset" banner when `editingListDefId` is set, plain text export
...
| ListDefinition | models/list-definition.ts | Dashboard list definition: name, sortOrder, `pinnedToDashboard`, membership (today / upcoming / deadlines / someday / custom), sort, grouping. v22: `seededKey` retired — renamed/deleted defaults stay that way. `today`/`upcoming` variants accept optional `warningWindowDays`; `custom` carries a `TodoPredicate` |
| TodoPredicate | models/filter-predicate.ts | Serializable predicate DSL mirroring the live filter fields (number arrays + ISO strings — no Sets, no Dates). Stored inside `ListMembership.custom` and `SavedViewFilters`; runtime converts to `FilterCriteria` via `predicateToCriteria` at evaluation time |
...
| ListInset | models/list-inset.ts | Canvas widget referencing a `ListDefinition` (`listDefinitionId` FK). v23 unified the two surfaces — `preset`/`attributeFilter`/`name` retired; every inset delegates to the dashboard-lists interpreter |
...
| SavedView | models/saved-view.ts | Named saved list view: sortBy + serializable filter snapshot (including dateRangeStart/End). Optional `maxTasks` (1..10000) + `limitMode` (`'hard' \| 'scroll'`) cap the visible task count per view |
| Todo2Database | data/database.ts | Dexie DB class with schema (v16 base + v17/v18/v19 incremental + v20 unified status + v21 unified scheduling + v22 list-definitions builder DSL + v23 canvas list-inset unification; v1-v15 collapsed) |
...
| ensureSeededListDefinitions | data/database.ts | Seeder for Today / Upcoming / Deadlines / Someday list-definition rows. v22 semantics: "insert iff table is empty" — seeded rows are normal rows; deletion is permanent |
| runV22Migration | data/database.ts | v22 upgrade: backfills `pinnedToDashboard=true` on every existing row; strips retired `seededKey` field |
| runV23Migration | data/database.ts | v23 upgrade: walks every existing `ListInset`, synthesizes an unpinned `ListDefinition` (via `buildListDefFromLegacyInset`), rewrites the inset to reference it by id, and strips `preset` / `attributeFilter` / `name`. Corrupt rows are dropped. |
| buildListDefFromLegacyInset | data/database.ts | Helper producing a `ListDefinition` shape from a pre-v23 inset row (preset → custom predicate with `dateRangeEnd=today+7d`; attributeFilter → personIds / tagIds / orgIds predicate). Shared by v23 migration and the file-import restore path. |
...
| useFilterStore | stores/filter-store.ts | Filter criteria (personIds, tagIds, orgIds, statusIds as null\|Set; showCompleted boolean, showHiddenStatuses boolean (overrides hideByDefault exclusion), searchText, dateField (date/scheduled/deadline/created/modified — `'date'` reads `effectiveDate`), dateRangeStart/End, dateRangeIncludeNoDate; personFilterMode (include-orgs/direct-only), orgFilterMode (include-people/direct-only)); displayed in TopBar filter bar |
| matchesFilter / applyFilter | stores/filter-store.ts | Top-level evaluators (not store methods as of v22). Take `filters: FilterCriteria` explicitly so the dashboard interpreter can synthesize a runtime FilterCriteria from a stored `TodoPredicate` for `ListMembership.custom` evaluation. `statuses[]` drives hideByDefault exclusion; optional `today` anchors fuzzy-scheduled resolution |
...
| useListDefinitionStore | stores/list-definition-store.ts | Dashboard list definitions store: load, add (appends with auto sortOrder), update, rename, setPinned (pinned=dashboard card), remove (with undo + snackbar), clone (unique-name copy), reorder (drag sort with persisted sortOrder). Exports `emptyPredicate()` for new custom lists |
| DashboardListsEditor | components/settings/DashboardListsEditor.tsx | Settings modal (StatusEditor pattern): dnd-kit reorderable list of list definitions, inline rename, pin-to-dashboard toggle, delete confirm, "Add list" footer. Per-row ⚙ toggle expands an inline `ConfigPanel` for live-edit of membership-kind / `warningWindowDays` / sort / sortBy / grouping. For `kind:'custom'`, renders predicate-summary chips (people/tags/orgs/statuses/date-range/show flags) and an "Edit in ListView…" deep-link that applies the predicate to the live filter store, restores `listSortBy`, sets `editingListDefId`, and navigates to /list. |
...
| truncateSections | views/ListView.tsx | Pure helper: walks an ordered `Section[]` and slices the tail so the total task count ≤ `maxTasks`; returns `{ displaySections, truncatedCount }`. Drives ListView's "hard" limit mode |
...
| DashboardView | views/DashboardView.tsx | Renders only `pinnedToDashboard` list definitions via `buildDashboardLists`; 2-column grid of `DashboardListCard`s; collapsible cards; drag tasks to taskboard via DndContext; appends an "Add list" tile (desktop) that opens `ListDefinitionPickerPopup` — pin an existing unpinned list or open `DashboardListsEditor` to create a new one |
| buildDashboardLists, interpretMembership, interpretSort, interpretGrouping, WARNING_WINDOW_DAYS | services/dashboard-lists.ts | Pure interpreter over `ListDefinition`. Membership kinds: today (≤ today OR deadline within `warningWindowDays`, default WARNING_WINDOW_DAYS=3), upcoming (has date, not in today — window threaded through for coupling), deadlines (has `dueDate`, intentional overlap with today), someday (no dates), custom (injected `ctx.evalPredicate` consumes a `TodoPredicate`). Sort kinds: effective-date-asc (expired-fuzzy first), deadline-asc, sort-order, sortBy (chronological `by` values sort properly; categorical fall back to sortOrder). Grouping kinds: none, relative-effective, relative-deadline, by-sortBy (chronological `sort.by` reuses the relative buckets; categorical returns undefined for now). Applies `showCompleted` + `showHiddenStatuses` gates. |
