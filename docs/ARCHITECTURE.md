...
├── stores/                → Zustand (canvas, canvas-rails, todo, project, person, tag, org, status, list-inset, floating-note, floating-calendar, taskboard, ui, filter, undo, saved-view, list-definition, settings, file-storage, note — note-store owns only the single global "outside-tasks" note; floating-note-store + floating-calendar-store own per-canvas placement widgets that view that global note / shared calendar filter)
