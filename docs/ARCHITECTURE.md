...
├── utils/                 → Shared pure utilities (hierarchy, dates, effective-date, filter — `toggleItem` + `matchTodoText(todo, query, ctx)`/`TextMatchField` for multi-field text search used by TopBar + `filter-store.matchesFilter`, platform — `isMacLike()` + `formatShortcut('Mod-t')` for ⌘/Ctrl-aware shortcut labels, slot-kind — `KIND_ICON` / `KIND_LABEL` maps, float-kind-switch — `convertFloatingKind` deletes a float row and recreates it at the target kind+rect, used by all four floating nodes on title-menu kind change)
