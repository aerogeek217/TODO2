...
| SchedulePicker | components/shared/SchedulePicker.tsx | Popover picker for `scheduledDate`: fuzzy chip options (today/tomorrow/this-week + secondary row next-week/this-month/next-month), Pick day for precise date, Clear; expired-fuzzy state shown with overdue marker |
