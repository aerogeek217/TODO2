...
├── stores/                → Zustand (canvas, canvas-rails, todo, project, person, tag, org, status, list-inset, floating-calendar, taskboard, ui, filter, undo, saved-view, list-definition, settings, file-storage, note — note-store owns both the global dashboard/rail note and per-canvas floating notes; floating-calendar-store owns per-canvas pop-out calendar widgets)
