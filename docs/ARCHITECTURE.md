...
│   ├── list-view/         → SavedListsControls (ListView's Save/Load chrome — `FavoriteChip` + `ListDefinitionSelector` + `NewListPrompt`; extracted from ListView in code-review-tail-2026-05-09 P2)
...
├── data/                  → Dexie repositories + restore + audit. Notable: `CURRENT_DB_VERSION` + `OLDEST_SUPPORTED_DB_VERSION` (moving floor; cross-floor loads fire `MigrationDialog` via `services/migration-check`); `todoEvents` (append-only history feeding stats); per-kind floating placement tables; audit detects unrecognized DB items (unknown tables / unknown rows / unknown setting keys) on top of orphan-join + dangling-FK checks
...
├── hooks/                 → `use-popover-anchor.ts` is the shared anchor + flip + clamp + scroll/resize/Esc/outside-click hook used by every popover; useFloatingWidget (header + dock/close/kind handlers + ResizeHandle); useThrottledResize (rAF); use-keyboard-shortcuts (skips elements with `data-shortcut-scope="none"`); use-grouped-task-blocks (SortableTaskList's grouped-render `useMemo` extracted in code-review-tail-2026-05-09 P5); use-entity-assignments-for-todos (people/org/tag triple-load on visible todo set; ListView + CalendarView + CanvasPage); use-sortable-row (`useSortableRow` + `useSortableReorderHandler` shared by HorizonRibbon / DashboardListsEditor / StatusEditor reorder rows)
├── utils/                 → sort-order (polymorphic `bySortOrder<T>` over any `{sortOrder, id?}` row), dates, effective-date (`resolveScheduled`/`resolveFuzzy`/`resolveFuzzyOrigin` — `resolveFuzzyOrigin` anchors aged-fuzzy on `ScheduledValue.fuzzy.setAt`; canonical `startOfWeek`; `compareTodosByDate(field, today, ws)` + `pickTodoDate` factories shared by ListView + dashboard-lists; `scheduledLabel` renders aged vocabulary), filter (`matchTodoText`/`TextMatchField`), platform (`isMacLike`/`formatShortcut`), debug-flags (`?debug-dnd=1` — permanent diagnostic infrastructure), slot-kind (`KIND_ICON`/`KIND_LABEL`), task-grouping (`partitionByGroup`/`getGroupColor`/`GROUP_OPTIONS`), cross-group-drag (`blockContextId`/`parseBlockContextId`/`resolveCrossGroupMutation`/`dispatchCrossGroupMutation`), rail-dnd (`RAILS_DRAG_KIND`/`resolveFloatDockTarget`/`nearestStubSlotId`/tab reducers), task-dnd/ (unified DnD vocabulary: `kinds.ts`, `ids.ts` per-surface drag ids, `collision.ts` declarative rule table, `dispatch.ts` single dispatcher for taskboard + calendar drops, `search-drop.ts`), canvas/ (alignment snap + cascade-shift — pure layout math used by `useCascadeShifts`)
└── services/              → NLP (parser + resolver; `#tag` lookup-or-create, case-insensitive), command registry, file storage, undoable, backup scheduler, ensure-default-project (Inbox auto-create), dashboard-lists interpreter (`buildDashboardLists` with multi-value `runtimeFilterValues`), list-view-sections (ListView's section builders + `truncateSections` + `itemSortComparator` + date-bucket helpers; extracted in code-review-tail-2026-05-09 P2), horizons (`classifyByDateSource`), stats/ (`status-breakdown`, `discipline`, `snooze`, `buckets`), notes-export (`mdToHtml`/`htmlToMarkdown`/`copyNotesRich`), task-copy (`copyTasksRich`/`buildTasksHtml`/`buildTasksPlain`), float-kind-registry (`FLOAT_KIND_REGISTRY` single-source-of-truth for the 8 floating widget kinds — `floatKindBySlotKind` / `floatKindByDragKind` / `floatKindForNodeId` / `isFloatNodeId`), float-kind-switch (`convertFloatingKind`)
```
