...
| RailsFrame | components/canvas/rails/RailsFrame.tsx | Outer rails shell. 3×3 CSS grid driven by `--left-size` / `--right-size` / `--top-size` / `--bottom-size` (0 when a rail is null); each rail is a grid item placed via `computeRailGridArea(rails, side)` so horizontal rails can extend into NW/NE/SW/SE corner cells when the corresponding `rails.corners[c] === 'h'`. `canvas-host` always occupies cell 2/2. Mounts the four `FrameCornerToggle` chevrons as direct absolute children at the canvas-host corners with arrow-key roving focus via the internal `CORNER_NAV` 2×2 wrap graph; renders `DockOverlay` on top while a rails drag is in flight. Hosts `useDefaultRails` (hydrates from `settings.canvasRails`, seeds a right-side `thisweek` lens on first run, debounce-persists back) and `useRailsDragMonitor` (routes `onDragEnd` to the matching store reducer + `applyEmptySideCorners(side, claim)` to wire the adjacent corners per the claimed/pinched semantics documented on `DockOverlay`). Also exports `popSlotToCanvas` / `popTabToCanvas` |
...
| FrameCornerToggle | components/canvas/rails/FrameCornerToggle.tsx | Tiny chevron button keyed by `Corner` (`nw`/`ne`/`sw`/`se`). Absolute-positioned at the inner corner of the matching corner cell via `--*-size` vars; hidden at rest (opacity 0) with a generous `::before` hit area that reveals the glyph on hover / `:focus-visible`. Click fires `onToggle(corner, next)` flipping ownership between `'v'` and `'h'`; `aria-pressed` reflects the `h` owner. Disabled (via `aria-disabled` + `pointer-events: none`) when either adjacent rail is null — ownership has nothing to trade. Arrow keys fire `onArrowNav(corner, key)` so the parent `RailsFrame` can move focus along its 2×2 wrap graph; Space/Enter toggle natively |
